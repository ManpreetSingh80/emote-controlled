...
class DistributionBasedClassifier(SentimentClassifier):
    ...
    def evaluate(self, twitch_data: pd.DataFrame):
        ...
        print("Macro Recall:\t" + "%.4f" % (
            recall_score(true_sentiment, pred["pred_sentiment"], average="macro")))
        print("Macro F1:\t" + "%.4f" % (
            f1_score(true_sentiment, pred["pred_sentiment"], average="macro")))
        print("Accuracy:\t" + "%.4f" % (accuracy_score(true_sentiment, pred["pred_sentiment"])))


if __name__ == '__main__':
    dbc = DistributionBasedClassifier()

    eval_data = "data/blizzcon18.csv"
    df = pd.read_csv(eval_data, keep_default_na=False)
    # dbc.evaluate(df)
    dbc.classify_df(df)
    df.to_csv('blizzcon18_out.csv')
